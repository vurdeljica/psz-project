...
cnx2 = sqlite3.connect('C:/Users/igvu/Desktop/discogs_baze/bazeTestiranje/discogs_new_all.db')
song_df2 = pd.read_sql_query("SELECT * FROM song", cnx2)
song_df['song_id_2'] = song_df2['song_id']
    ...
    while len(list_of_jobs_per_thread) > 0:
        ...
        while num_of_exceptions < 100:
            try:
                ...
                url_full = requests.get('https://www.discogs.com/track/' + url_base, proxies=proxies, headers=headers, timeout=20)
        ...
        if (url_full != '--')  and ("Response" not in url_full):
...
for index, row in song_df.iterrows():
    if '/composition/' not in row['song_id']:
        list_of_song_id.append(row['song_id_2'])
...
#                }
#
#                url_full = requests.get('https://www.discogs.com/track/' + row['song_id_2'], proxies=proxies, headers=headers, timeout=3)
#                status_code = url_full.status_code
#
#               if status_code == 429 or "Response" in url_full:
#                    raise ValueError('429 Error')
...
#                done += 1
#                total += 1
#                song_df.loc[song_df['song_id_2'] == row['song_id_2'], 'song_id'] = url_full
#                conn.close()
#
#                break
#            except Exception as e:
...
#           total += 1
#            print(done, "/", total)
#           song_df.loc[song_df['song_id_2'] == row['song_id_2'], 'song_id'] = row['song_id_2']
#
#del song_df['song_id_2']
#song_df = song_df.astype(str)
#song_df.to_sql('song', cnx, if_exists='replace', index=False)